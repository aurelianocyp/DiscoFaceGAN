...
def z_to_lambda_mapping(latents,exp=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        with tf.variable_scope('id'):
            IDcoeff = CoeffDecoder(z=latents[:, :128], coeff_length=160, ch_dim=512, ch_depth=3)
            if exp!=None:
                mat_data = scipy.io.loadmat(exp)
                IDcoeff1 = tf.convert_to_tensor(mat_data["id"], dtype=tf.float32)
                IDcoeff2 = tf.convert_to_tensor(mat_data["tex"], dtype=tf.float32)
                IDcoeff =tf.concat([IDcoeff1,IDcoeff2],axis=1)
        ...
        with tf.variable_scope('exp'):
             ...
             if exp!=None:
                mat_data = scipy.io.loadmat(exp)
                EXPcoeff = tf.convert_to_tensor(mat_data["exp"], dtype=tf.float32) #print("EXPcoeff", type(EXPcoeff))
                
        with tf.variable_scope('gamma'):
            GAMMAcoeff = CoeffDecoder(z=latents[:, 128 + 32:128 + 32 + 16], coeff_length=27, ch_dim=128, ch_depth=3)
            #if exp!=None:
             #   mat_data = scipy.io.loadmat(exp)
              #  GAMMAcoeff = tf.convert_to_tensor(mat_data["gamma"], dtype=tf.float32)
        with tf.variable_scope('rot'):
            Rotcoeff = CoeffDecoder(z=latents[:, 128 + 32 + 16:128 + 32 + 16 + 3], coeff_length=3, ch_dim=32,ch_depth=3)
            if exp!=None:
                mat_data = scipy.io.loadmat(exp)
                Rotcoeff = tf.convert_to_tensor(mat_data["angle"], dtype=tf.float32)
                #print("EXPcoeff", type(EXPcoeff))
...
def truncate_generation(Gs, inputcoeff, rate=0.7, dlatent_average_id=None):
    ...
    # fake = tflib.run(fake_images_out)
    # fake_images_np = fake_images_out.numpy()
...
def parse_args():
    ...
    parser.add_argument('--subject', type=int, default=1, help='how many subjects to generate.')
    parser.add_argument('--variation', type=int, default=500, help='how many images to generate per subject.')
...
def main():
    ...
    annotations = glob('epoch_20_000000/*.mat')
    for i in tqdm(annotations):
        with tf.device('/gpu:0'):
            ...
            # Pick latent vector.
            latents = tf.placeholder(tf.float32, name='latents', shape=[1, 128 + 32 + 16 + 3])
            #print("latents.shape:", latents.shape)  # latents.shape: (1, 179)
            noise = tf.placeholder(tf.float32, name='noise', shape=[1, 32])
            INPUTcoeff = z_to_lambda_mapping(latents,exp=i)
            #print("INPUTcoeff", INPUTcoeff.shape)  # INPUTcoeff (1, 254)
            ...
            #print(type(fake_images_out))
            #print(fake_images_out[0])
            #print(fake_images_out.shape)
            img = tf.cast(fake_images_out[0], dtype=tf.uint8)
            img = tf.image.encode_png(img)
            # img.numpy()
            # print(img)
            # PIL.Image.fromarray(fake_images_out[0].astype(np.uint8), 'RGB').save(os.path.join(save_path,'111.pmg'))
            restore_weights_and_initialize()
            #np.random.seed(1)
            lats1 = np.zeros((1, 128 + 32 + 16 + 3))  # 179
            noise_ = np.zeros((1, 32))  # 32
            lats2 = np.zeros((1, 32 + 16 + 3))  # 51
            ...
            PIL.Image.fromarray(fake[0].astype(np.uint8), 'RGB').save(os.path.join(save_path, f'{len(os.listdir(save_path))}.png'))
